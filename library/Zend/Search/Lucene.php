...
class Zend_Search_Lucene implements Zend_Search_Lucene_Interface
{
	...
	public function getIndexWritter1()
	{
		return $this->_getIndexWriter();
	}
    /**
    ...
    public function find($query)
    {
        ...
        foreach ($query->matchedDocs() as $id => $num) {
            ...
            if( $docScore != 0 ) {
                ...
                $hits[]   = $hit;
				$valuesArray[] = $hit->getDocument()->getFieldValue($sortField);
				$ids[]    = $id;
                $scores[] = $docScore;
...
*/
			...
			{
				 if (self::$_resultSetLimit != 0  &&  count($hitsNew) >= self::$_resultSetLimit) {
				 	break;
				 }
				$hitsNew[]	=	$value;
			}
