<?php 
class Business_VendorController extends Zend_Controller_Action {
	/**
     * @var result
    */
	protected $result;
    ...
    */
	...
	public function indexAction() {
		if(!Zend_Session::namespaceIsset('sess_login')) {
			...
			if(Zend_Session::namespaceIsset('delete_success_vendor')) {
				$this->view->success = 'Vendor deleted successfully';
				Zend_Session::namespaceUnset('delete_success_vendor');
			}
			if(Zend_Session::namespaceIsset('delete_error_vendor')) {
				$this->view->error = 'Vendor is already associated with expense so unable to delete';
				Zend_Session::namespaceUnset('delete_error_vendor');
			}
			if(Zend_Session::namespaceIsset('insert_success_vendor')) {
				$this->view->success = 'Vendor Created Successfully';
				Zend_Session::namespaceUnset('insert_success_vendor');
			}
			$delid   = base64_decode($this->_getParam('delid'));
			$delname = $this->_getParam('delname');
			if(isset($delid) && !empty($delid)) {
				$deleteStatus = $this->business->deleteVendor($delid);
				$auditLog  = $this->settings->insertAuditLog(3,7,$delname,$delid);
				if($deleteStatus && $deleteStatus==1) {
					$sessSuccess = new Zend_Session_Namespace('delete_success_vendor');
					$sessSuccess->status = 1;
					$this->_redirect('business/vendor');
				} elseif ($deleteStatus && $deleteStatus==3) {
					$sessError = new Zend_Session_Namespace('delete_error_vendor');
					$sessError->status = 1;
					$this->_redirect('business/vendor');
				}
			}
			$this->view->result 	=  $this->business->getVendors();
	...
	public function addAction() {
		if(!Zend_Session::namespaceIsset('sess_login')) {
			...
			$this->view->vendor_id   = $this->business->getVendorID();
			$this->view->otherPayable = $this->business->getOtherPayables();
			if(isset($logSession->proxy_cid) && !empty($logSession->proxy_cid)) {
				$cid = $logSession->proxy_cid;
			} else {
				$cid = $logSession->cid;
			...
			if($this->_request->isPost()) {
				...
				$this->vendor = $this->business->insertVendor($postArray,$cid);
				$auditLog  = $this->settings->insertAuditLog(1,7,$postArray['vendor_name'],$this->vendor);

				if(isset($this->vendor) && !empty($this->vendor)) {
					$sessSuccess = new Zend_Session_Namespace('insert_success_vendor');
				    $sessSuccess->status = 1;
					$this->_redirect('business/vendor/');	
				} else {
					$this->view->error = 'Vendor cannot be created. Kindly try again later';
				}
			}
		}
	}
	...
	public function editAction() {
		if(!Zend_Session::namespaceIsset('sess_login')) {
			...
			if(Zend_Session::namespaceIsset('update_success_vendor')) {
				$this->view->success = 'Vendor Updated Successfully';
				Zend_Session::namespaceUnset('update_success_vendor');
			}
			$logSession = new Zend_Session_Namespace('sess_login');
			$getAccountArray         =  $this->accountData->getData(array('country','currencies'));
			$this->view->countries   =  $getAccountArray['country'];
			...
			if(!isset($id) || $id=='') {
				...
				$this->view->vendor  =  $this->business->getvendors($id);
				//echo '<pre>'; print_r($this->view->vendor); echo '</pre>';
				if(!$this->view->vendor) {
					$this->_redirect('business/vendor/');
				} else {
					$this->view->contacts  =  $this->business->getKeyContacts($id,2);
					$this->view->expense   =  $this->business->getExpenseCount($id);
			...
			if($this->_request->isPost()) {
				$postArray  =   $this->getRequest()->getPost();
				//$postArray['other_coa'] = implode(",", $postArray['other_coa_link']);
				$this->vendor = $this->business->updateVendor($postArray,$id);
				$auditLog  = $this->settings->insertAuditLog(2,7,$postArray['vendor_name'],$id);

				if(isset($this->vendor) && !empty($this->vendor)) {
					$sessSuccess = new Zend_Session_Namespace('update_success_vendor');
				    $sessSuccess->status = 1;
					$this->_redirect('business/vendor/edit/id/'.$this->_getParam('id'));	
				} else {
					$this->view->error = 'Vendor cannot be updated. Kindly try again later';
				}
			}
		}
	}
		...
		if(!Zend_Session::namespaceIsset('sess_login')) {
			...
			if($this->_request->isPost()) {
				...
				if(isset($this->vendor) && !empty($this->vendor)) {
					...
					$this->view->error = 'Vendor cannot be added. Kindly try again later';
				}
			}
		}
	}
	...
	public function receiptsAction() {
		if(!Zend_Session::namespaceIsset('sess_login')) {
			...
			if(Zend_Session::namespaceIsset('delete_success_receipt_vendor')) {
				$this->view->success = 'Receipts deleted successfully';
				Zend_Session::namespaceUnset('delete_success_receipt_vendor');
			}
			if(Zend_Session::namespaceIsset('delete_error_receipt_vendor')) {
				$this->view->error = 'Receipts already associated in expense so unable to delete';
				Zend_Session::namespaceUnset('delete_error_receipt_vendor');
			}
			...
			if(!isset($id) || $id=='') {
				...
				$this->view->vendor  =  $this->business->getVendorDetails($id);
				//echo '<pre>'; print_r($this->view->vendor); echo '</pre>';
				if(!$this->view->vendor) {
					$this->_redirect('business/vendor/');
				} else {
					$this->view->result  =  $this->business->getReceipts($id,2);
					//echo '<pre>'; print_r($this->view->result); echo '</pre>';
				}
			}
			...
			if(isset($delid) && !empty($delid) && isset($receipt) && !empty($receipt)) {
				$checkReceipt = $this->business->checkReceipt($delid,2);
				if($checkReceipt==0) {
				if(unlink($this->fileuploadpath.$receipt)) {
					$deleteStatus = $this->business->deleteReceipt($delid);
					if($deleteStatus) {
						$sessSuccess = new Zend_Session_Namespace('delete_success_receipt_vendor');
						$sessSuccess->status = 1;
					}
						$this->_redirect('business/vendor/receipts/id/'.$this->_getParam('id'));
				} else {
					$this->view->error = 'Receipt cannot be deleted. Kindly try again later';
				}
				} else {
					$sessSuccess = new Zend_Session_Namespace('delete_error_receipt_vendor');
					$sessSuccess->status = 1;
					$this->_redirect('business/vendor/receipts/id/'.$this->_getParam('id'));
				}
			}
	...
	public function receiptAddAction() {
		if(!Zend_Session::namespaceIsset('sess_login')) {
			...
			if(Zend_Session::namespaceIsset('insert_success_vendor_receipt')) {
				$this->view->success = 'Receipt Uploaded Successfully';
				Zend_Session::namespaceUnset('insert_success_vendor_receipt');
			}
			$logSession = new Zend_Session_Namespace('sess_login');
			$this->fileuploadpath    =  $this->receiptPath.$logSession->cid."/";
			...
			if($this->_request->isPost()) {
				...
				if ($adapter->isValid('file') && isset($postArray['extension']) && !empty($postArray['extension']) && isset($postArray['attach_file']) && !empty($postArray['attach_file'])) {
					...
					$this->vendor = $this->business->insertReceipt($postArray,$id,2);

					if(isset($this->vendor) && !empty($this->vendor)) {
						$sessSuccess = new Zend_Session_Namespace('insert_success_vendor_receipt');
					    $sessSuccess->status = 1;
						$this->_redirect('business/vendor/receipt-add/id/'.$this->_getParam('id'));	
					} else {
	...
	public function ajaxCallAction() {
		...
		if($this->_request->isXmlHttpRequest()) {
			if ($this->_request->isPost()) {
				...
				if($ajaxVal['action']=='check_vendor') {
					$checkVendor = $this->business->checkVendor($ajaxVal['vendor_name']);
					if($checkVendor) {
						echo "2";
					} else {
						echo "1";
					}
				} else if($ajaxVal['action']=='check_vendor_update') {
					$checkVendor = $this->business->checkVendor($ajaxVal['vendor_name'],$ajaxVal['vendor_id']);
					if($checkVendor) {
						echo "2";
					} else {
						echo "1";
